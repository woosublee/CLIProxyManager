...
[[ "$(make -s -C "$REPO_ROOT" print-app-version)" == '0.1.35' ]] || fail "Makefile version must delegate to resolver"
[[ "$(make -s -C "$REPO_ROOT" print-build-number)" == '38' ]] || fail "Makefile build must delegate to resolver"
[[ "$(make -s -C "$REPO_ROOT" print-build-tag)" == 'v0.1.35' ]] || fail "Makefile tag must delegate to resolver"
...
printf '%s\n' "$canonical_dmg_dry_run" | grep -F 'release-output/CLIProxyManager-0.1.35.dmg' >/dev/null ||
...
printf '%s\n' "$development_bundle_dry_run" | grep -F \
  'make verify-app-structure ARTIFACT_CHANNEL=development CONFIGURATION=debug DEVELOPMENT_VERSION="0.1.35" DEVELOPMENT_BUILD_NUMBER="38"' \
...
printf '%s\n' "$development_bundle_dry_run" | grep -F \
  'scripts/verify-app-structure.sh --app "build/CLIProxyManager.app" --version "0.1.35" --build "38" --channel "development"' \
...
sign_structure_line="$(printf '%s\n' "$sign_dry_run" | grep -n -m 1 -F 'scripts/verify-app-structure.sh --app "build/CLIProxyManager.app" --version "0.1.35" --build "38" --channel "official"' | cut -d: -f1)"
...
development_verify_dry_run="$(
  make -n -C "$REPO_ROOT" verify \
    ARTIFACT_CHANNEL=development \
    DEVELOPMENT_VERSION=0.1.35 \
    DEVELOPMENT_BUILD_NUMBER=38
