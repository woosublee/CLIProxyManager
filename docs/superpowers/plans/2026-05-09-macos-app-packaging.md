...
INSTALLED_HELPER = /usr/local/bin/cliproxy-manager
...
bundle: swift-build Info.plist CLIProxyManager.entitlements
	@test -x "$(APP_EXECUTABLE)" || (echo "Missing app executable: $(APP_EXECUTABLE)" >&2; exit 1)
	@test -x "$(HELPER_EXECUTABLE)" || (echo "Missing helper executable: $(HELPER_EXECUTABLE)" >&2; exit 1)
	@rm -rf "$(APP_BUNDLE)"
...
sign: bundle
	...
	@codesign --force --options runtime --sign "$(CODESIGN_IDENTITY)" --entitlements CLIProxyManager.entitlements "$(APP_BUNDLE)" || \
		(echo "codesign failed. Try: make all CODESIGN_IDENTITY=\"Apple Development: Your Name (TEAMID)\"" >&2; exit 1)
...
install-helper: sign
	@mkdir -p "$(dir $(INSTALLED_HELPER))" || \
		(echo "Cannot create $(dir $(INSTALLED_HELPER)). Check local permissions." >&2; exit 1)
	@cp "$(HELPER_EXECUTABLE)" "$(INSTALLED_HELPER)" || \
		(echo "Cannot install helper to $(INSTALLED_HELPER). Check local permissions." >&2; exit 1)
	@chmod +x "$(INSTALLED_HELPER)"
	@echo "Installed helper to $(INSTALLED_HELPER)"

install: sign install-helper
	@rm -rf "$(INSTALLED_APP).staging"
	@ditto "$(APP_BUNDLE)" "$(INSTALLED_APP).staging" || \
		(echo "Cannot stage app at $(INSTALLED_APP).staging" >&2; rm -rf "$(INSTALLED_APP).staging"; exit 1)
	@rm -rf "$(INSTALLED_APP).previous"
	@if [ -d "$(INSTALLED_APP)" ]; then mv "$(INSTALLED_APP)" "$(INSTALLED_APP).previous"; fi
	@mv "$(INSTALLED_APP).staging" "$(INSTALLED_APP)" || \
		(echo "Cannot install app to $(INSTALLED_APP)" >&2; \
		 if [ -d "$(INSTALLED_APP).previous" ]; then mv "$(INSTALLED_APP).previous" "$(INSTALLED_APP)"; fi; \
		 rm -rf "$(INSTALLED_APP).staging"; exit 1)
	@rm -rf "$(INSTALLED_APP).previous"
...
Installed CLIProxyManager.app to /Applications
```

If helper installation fails due to permissions, rerun with a writable `/usr/local/bin` setup or use a shell that can create/write that path.
