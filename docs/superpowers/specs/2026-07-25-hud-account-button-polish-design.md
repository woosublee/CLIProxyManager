...
또한 Expanded HUD에서 숨긴 계정을 다시 표시할 때 새로 추가되는 계정 행만 순간적으로 깜빡인다. 계정을 숨길 때와 Compact 모드에서는 같은 문제가 관찰되지 않았다.
...
4. Expanded HUD에서 계정을 다시 표시할 때 새 행의 순간적인 깜빡임을 짧은 fade-in으로 완화한다.
5. 숨김 동작, Compact HUD, 저장·필터·오류 처리 동작은 변경하지 않는다.
...
## Expanded 계정 insertion 전환

### 관찰된 증상

Expanded HUD에서 계정을 다시 표시하면 기존 계정은 안정적으로 유지되지만 새로 추가되는 계정 행만 순간적으로 번쩍이거나 잘렸다 나타난다. 제거 시에는 문제가 없고 Compact에서도 관찰되지 않았다.

Expanded provider 목록은 계정 추가와 함께 자연 높이가 증가하고 AppKit panel이 fitting size로 조정된다. 새 행이 즉시 완전 불투명으로 그려지면 기존 panel 높이에서 새 높이로 맞춰지는 한 프레임의 시각 변화가 깜빡임처럼 보일 수 있다.

### 채택 동작

- Expanded provider ID 목록에 새 ID가 추가될 때 새 계정 행에만 opacity insertion transition을 적용한다.
- duration: `0.12`초
- timing: `easeOut`
- removal transition: identity — 숨길 때는 현재처럼 즉시 제거한다.
...
SwiftUI 구현은 account row에 asymmetric transition을 적용하고 provider ID 배열을 animation value로 사용한다.
...
실제 구현에서는 animation scope가 Expanded account stack에만 한정되어야 한다. Usage HUD의 기존 display-mode blur/opacity transition에는 영향을 주지 않는다.

## Reduce Motion

`accessibilityReduceMotion`이 활성화되면 insertion animation을 사용하지 않는다. 계정 행과 panel size는 최종 상태로 즉시 갱신한다.

Compact mode는 이 설정과 관계없이 이번 변경 전 동작을 유지한다. 기존 mode transition의 Reduce Motion 정책도 변경하지 않는다.
...
- Expanded account row에 asymmetric transition이 존재한다.
- insertion은 opacity이고 removal은 identity다.
- animation duration은 `0.12`, timing은 `easeOut`이다.
- animation value는 provider ID 배열이다.
- 전체 account stack이나 header에 crossfade를 적용하지 않는다.
- Compact view에는 이번 insertion animation을 추가하지 않는다.
- Reduce Motion에서는 animation이 nil이다.
...
- Expanded에서 다시 켤 때 새 계정만 부드럽게 나타나고 기존 계정은 깜빡이지 않는지
- Compact 동작이 이전과 동일한지
...
- Expanded에서 새로 추가되는 계정 행만 120ms fade-in한다.
- 제거, Compact, Reduce Motion 동작이 명시된 정책을 따른다.
